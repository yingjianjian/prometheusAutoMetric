...
urlCollector = Gauge("watcherStatus","watcher Url status",["status","code"],registry=REGISTRY_URL)
...
class urlStatus(View):
    def get(self,request):
        code = urlRequest.isciasStatus('http://111.0.120.180:38080/iscias/project/page/2019ncov')
        if code >= 400 and code <=599:
            status = "failed"
            urlCollector.labels(status, code).set(0)
        else:
            status = "success"
            urlCollector.labels(status,code).set(1)
